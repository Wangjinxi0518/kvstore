...
void Conn::Splice() {    // 将数字字符串转为整形数字
    ...
    if (state_ == kBluk) {
        bluk_length_ =
            atoi(buffer_.substr(analysis_pos_, read_pos_ - analysis_pos_).c_str());    // *后的数字字符串转换为整形数字
        // LOG(INFO) << "bluk_length_: " << bluk_length_;
        Assert();    // 处理'\r','\n'的情况
    } else if (state_ == kMultiBluk) {
        left_ = multibluk_length_ =
            atoi(buffer_.substr(analysis_pos_, read_pos_ - analysis_pos_).c_str());    // $后的数字字符串转换为整形数字
...
void Conn::Memmove() {
    // LOG(INFO) << "read_pos_num: " << read_buffer_[read_pos_] << "  read_pos_num
    // - 1: "  << read_buffer_[read_pos_ - 1]; LOG(INFO) << "read_pos: " <<
    // read_pos_ << "  read_buffer_size_: " << read_buffer_size_;
    offset_ = read_buffer_size_ - read_pos_;    // 计算menmove的值
    memmove(read_buffer_, read_buffer_ + read_pos_,
            sizeof(char) * offset_);    // [0][1][2][3][4] -> [2][3][4][x][x]
    read_buffer_size_ = offset_;
    analysis_pos_ = read_pos_ = 0;
    // LOG(INFO) << "offset_: " << offset_ << "  read_buffer_size_: " <<
    // read_buffer_size_ << "strlen(read): " << strlen(read_buffer_);
}

void Conn::Finterpreter() {    // 解析序列化后的数据
    // LOG(INFO) << "after_read_buffer_: " << read_buffer_;
    // LOG(INFO) << "after_read_buffer_size_: " << strlen(read_buffer_);
    is_end_parsing_ = false;

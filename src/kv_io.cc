/**
 * @File Name: kv_io.cc
 * @brief : 这段代码使用了C++的文件I/O操作，字符串处理和系统调用等技术来实现一个简单的键值存储。技术难点可能在于理解文件I/O操作和字符串处理的细节，以及如何正确地使用系统调用。
...
void IO::set(std::string order, std::string key,
             std::string value) {    // 文件set指令
    int maxpos;
    std::ofstream ofs;
    std::string sql = "";
...
std::string IO::Extract(std::string templine,
                        int &position) {    //文件中提取字符串中的key，value
    std::string word = "";
    std::string str = "";
    int size;

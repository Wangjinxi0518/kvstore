/**
 * @File Name: echoserveri.cc
 * @brief: 这段代码是一个键值存储服务器的实现，主要使用了C++、Epoll事件驱动模型、多路复用、套接字编程、日志库glog、以及一些自定义的类和函数。
    ...
    while (keepRunning) {
        ...
        for (int index = 0; index < event_total; index++) {
            if (ClusterEpoll::JudgeFirst(index, listenfd)) {
                ...
                assert(conn != nullptr);
                conn->ProcessNewConn(
                    listenfd);    // 这里的Coon以及Coon*我用的是栈变量，用完即释放，保证每个对象的安全性
                ...
                if (conn_map.find(ClusterEpoll::GetFD(index))
                    != conn_map.end()) {    // 判断获取到的fd是否存在于ConnectionMap集合中
